...
def train_LSTM_model():
    ...
    feature_col_market_factor = ['preclose', 'open', 'close', 'volume', 'high', 'low']
    ...
    feature_col = feature_col_market_factor
    ...
    # 2 by XGB_HMM
    # 2.1 hangqing
    solved_dataset1, allow_flag1 = HMM_MarketFactor.solve_on_raw_data(dataset, lengths, feature_col)
    temp = pickle.load(open('/content/drive/MyDrive/Projects/Database/Models/market_XGB_HMM_model.pkl', 'rb'))
    ...
    final_X, final_y, final_lengths = pred_proba1, label, lengths #combine(pred_proba1, pred_proba2, allow_flag1, allow_flag2, label, lengths)
