...
def solve1(dataset, lengths, feature_col):
    ...
    for i in range(len(lengths)):
        ...
        closePrice = dataset[begin_index:end_index, feature_col.index('close')]
        vol = dataset[begin_index:end_index, feature_col.index('volume')]
        highestPrice = dataset[begin_index:end_index, feature_col.index('high')]
        lowestPrice = dataset[begin_index:end_index, feature_col.index('low')]
...
def solve2(dataset, feature_col):
    ...
    result[:, 0] = dataset[:, feature_col.index('open')]/dataset[:, feature_col.index('preclose')]
    result[:, 1] = dataset[:, feature_col.index('low')]/dataset[:, feature_col.index('preclose')]
    result[:, 2] = dataset[:, feature_col.index('high')]/dataset[:, feature_col.index('preclose')]
    result[:, 3] = dataset[:, feature_col.index('close')]/dataset[:, feature_col.index('preclose')]
