...
def train_HMM_model(n_states):
    ...
    # if not (os.path.exists('/content/drive/MyDrive/Projects/Database/Models/market_GMM_HMM_model.pkl') and 
    if not os.path.exists('/content/drive/MyDrive/Projects/Database/Models/market_XGB_HMM_model.pkl'):
        feature_col = ['preclose', 'open', 'close', 'volume', 'high', 'low']
        dataset, label, lengths, col_nan_record = form_raw_dataset(feature_col, label_length=5)
        solved_dataset, allow_flag = HMM_MarketFactor.solve_on_raw_data(dataset, lengths, feature_col)
        ...
        # pickle.dump(temp, open('/content/drive/MyDrive/Projects/Database/Models/market_XGB_HMM_model.pkl', 'wb'))

        # 1.3 train and save the XGB_HMM model
        print('training market factor XGB_HMM model...')
        A, xgb_model, pi = XGB_HMM(X_train, lengths_train)
        pickle.dump([A, xgb_model, pi], open('/content/drive/MyDrive/Projects/Database/Models/market_XGB_HMM_model.pkl', 'wb'))
    ...
    #     pickle.dump(XGB_model_list, open('/content/drive/MyDrive/Projects/Database/Models/market_XGB_HMM_model.pkl', 'wb'))
